  ...
  <div class="row justify-content-center">
    <div class="col-md-12">
      <div id="report-header" class="d-flex mb-2 justify-content-between">
        <h1>Importación de lotes y/o zonas al proyecto <b>{{$project->nombre}}</b>.</h1>
      </div>
  ...
  @if($lots !== null)
    <script>
      ...
      for (let i = 0; i < mainLots.length; i++) {
        ...
        if(e['missingClient']){
          let missingClient = `<ul><li>No se localizó un cliente registrado con el correo electrónico incluido: <b>${e.email}</b>.</li></ul>`
          $(missingClient).appendTo($(`#lote${i}`));
        }
      }
  ...
  @if($zones !== null)
    <script>
      ...
      for (let i = 0; i < mainZones.length; i++) {
        const e = mainZones[i];
        let listElement = `<li>Se agregó la zona <b>${e.etapa}</b> <b>"${e.e_name}"</b>.`;
  ...
  @if($fails !== null)
    <script>
      ...
      for (let i = 0; i < mainFails.length; i++) {
        ...
        if(e['missingClient']){$(`#e-list${i}`).append(`<li>No se localizó un cliente registrado con el correo electrónico incluido: <br>${e.correo_cliente}</b> .</li>`);}
      }
